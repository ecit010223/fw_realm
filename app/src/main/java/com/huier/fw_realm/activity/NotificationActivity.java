 ...
 * 可以通过调用removeChangeListener()、removeAllChangeListeners()来停止通知,侦听器所注册的相应对象被垃圾回
 * 收时，通知也会停止,请在所期望的通知生命周期内，保持对侦听器注册的相应对象的强引用。
 * 通知回调函数永远只会在注册通知的对应线程上进行调用，并且该线程需要一个正在运行的Looper。
 * 如果相关的写操作发生在一个另外的线程之上，那么侦听器会在事务提交后被异步调用。
 * 如果相关写操作发生在相同线程之上，那么侦听器会在事务提交时被同步调用。如果当本线程中的Realm数据不是最新版本，
 * 且最新版本的数据改变与当前注册的监听器相关，那么当事务开始(beginTransaction())的时候，侦听器也会被同步调用。
 * 这些情况里侦听器会被beginTransaction()和commitTransaction()同步调用，这意味着这时在回调函数中开启事务
 * (beginTransaction())会由于事务嵌套的原因抛出异常。假如你的应用架构有可能触发这种情况，请通过
 * Realm.isIntransaction()来判断是否当前Realm实例的事务状态。
 * 因为异步的通知是通过looper事件来传递的，队列中的其他无关事件有可能因为通知送达延迟。这意味着多个事务中改变的
...
public class NotificationActivity extends AppCompatActivity {
    ...
    /**
     * 当后台线程向Realm添加数据，你的UI线程或者其它线程可以添加一个监听器来获取数据改变的通知,监听器在Realm数
     * 据改变的时候会被触发。
     */
    ...
    /**
     * 集合通知与Realm通知不同,它会包含针对那些集合数据改变的信息,这些信息会指明从上次通知到达以来,集合中的插入、
     * 删除以及改变的元素索引。
     * 对于异步查询(findAllAsync())来说通知第一次到达时改变集合为空,表明了这是异步查询完成后的第一次通知,之后
     * 的通知则包含相应的集合改变信息。
     * 可以通过传递给监听器的OrderedCollectionChangeSet对象来获取集合中删除(deletions)、插入(insertions)和
     * 修改(changes)的元素索引。
     * 插入(insertions)和删除(deletions)指有元素被添加进或移除出集合从而引起集合元素构成的改变,这些改变有可能
     * 在创建或者删除相关Realm对象时发生,对于RealmResults来说，当修改了某个Realm对象的属性,而这个属性修改会导
     * 致查询结果的不同,这时集合的插入和修改通知也会被触发。
     * 修改(changes)通知会当集合中的某对象的属性改变时被触发,当然这个属性的改变需能保证该元素仍然满足查询条件从
     * 而使其仍然存在于集合当中。
     */
     ...
     * 可以通过监听器传递的ObjectChangeSet来获取对象属性改变的详细信息，包括哪些属性被修改以及该对象已被删除。
     * 在监听对象被删除时ObjectChangeSet.isDeleted()会返回true。
     * ObjectChangeSet.getChangedFields()会返回哪些字段有所改变,也可以通过ObjectChangeSet.isFieldChanged()
     * 来判断一个指定字段是否改变。
     */
