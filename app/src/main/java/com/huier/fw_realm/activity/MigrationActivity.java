...
public class MigrationActivity extends AppCompatActivity {
    ...
    /**
     * Realm文件可以通过传递一个512位(64字节)的密钥参数给Realm.getInstance().encryptionKey()
     * 来加密存储在磁盘上。这保证了所有永久性存储在磁盘上的数据都是通过标准AES-256加密的,每次创
     * 建新的Realm实例的时候,都需要提供相同的密钥。
     */
...
//        Realm realm = Realm.getInstance(config);
    ...
    /**
     * 如果没有旧Realm数据文件存在，那么迁移并不需要，在这种情况下，Realm会创建一个新的以.realm为后缀，
     * 基于新的对象模型的数据文件。在开发和调试过程中，假如你需要频繁改变数据模型，并且不介意损失旧数据，
     * 你可以直接删除.realm文件(这里包含所有的数据)而不用关心迁移的问题。
     * 这在你应用的开发早期阶段非常有用。
     */
