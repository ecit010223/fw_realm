...
public class DocumentActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    /**
     * 快照
     * 所有的Realm集合都会自动更新,它们总会被更新到最新的数据,这在大多数情况下是符合预期的,但是在遍历并且同时
    ...
    private void snapshots(){
        ...
        /**
         * 这段代码预期通过一个简单循环来邀请所有的Guest对象，因为RealmResults在每次循环都会被更新，你会发现
         * 最终的运行结果是只有一半的Guest对象收到了邀请，当Guest对象收到邀请以后，它会被立即从集合中移除，
         * 因为它不再满足查询条件，集合的大小在此刻发生了改变，所以当i增加时，循环会错过一个集合元素。
         * 可以通过使用集合数据的快照来解决这个问题，集合快照保证其中的元素及其顺序不会改变，即使在元素被修改
         * 甚至删除的情况下。
...
//        });
    ...
    /**
     * 事务会相互阻塞其所在的线程，在后台线程中开启事务进行写入操作可以有效避免UI线程被阻塞。通过使用异步事务，
     * Realm会在后台线程中进行写入操作，并在事务完成时将结果传回调用线程。
...
//        mRealm.cancelTransaction();
    ...
    private void linkQuery(){
         ...
         * 然后返回这两个结果的交集.
         * 两个条件分别是equalto("dogs.name", "Fluffy")和equalto("dogs.color","Brown")。u1和u2完全满足
         * 第一个条件 —— 我们称其c1集合,u1和u2也同时完全满足第二个条件 —— 我们称其c2集合,查询中的逻辑与即是
         * c1与c2的交集,c1与c2的交集就是u1和u2.
         * 会查询出所有的PersonLink、所有的DogLink
         */
         ...
         * u2（两个Person都有颜色为brown的 Dog）,最后的查询r2 = r2b.where().equalto("dogs.color", "yellow").findAll();
         * 结果只包含u2，因为只有u2同时有一个颜色为Brown的Dog和一个颜色为Yellow的 Dog。
         * 会查询出U2
         */
