...
/**
 * 可以实时的在不同线程中读取和写入Realm对象,不用担心其它线程会对同一对象进行操作,只需要在改变对象时
 * 使用事务,在另一线程中指向同一对象的数据会被即时更新(更新会在下一次事件循环时进行)。
 * 唯一局限是不能随意跨线程传递Realm对象,如果在另一线程使用同一对象,请在那个线程使用查询重新获得该对
 * 象,请谨记所有的Realm对象都会在不同线程中保持更新,Realm会在数据改变时通知你。
 * 跨线程使用Realm:
 * 请谨记：Realm、RealmObject和RealmResults实例都不可以跨线程使用,但是可以使用异步查询和异步事务来
 * 将部分操作放入后台线程进行，待完成时调用线程被通知以获取结果。
 * 当需要跨线程访问同一部分数据时，只需简单地在该线程重新获取一个Realm实例(例如：
 * Realm.getInstance(RealmConfiguration config)或是其他类似方法)，然后通过这个Realm实例来查询获得
 * 你需要的数据,查询获得的对象会映射到Realm中的相同数据，由此方法获得对象在其线程中任何地方都可读写！
 * 如下实例：
 * 假设我们的应用要展示一个用户列表。我们在一个后台线程中(一个安卓 IntentService)从远端获取新用户并
 * 将它们存储到Realm中,但后台线程存储新用户时，UI线程中的数据会被自动更新。UI线程会通过
 * RealmChangeListener得到通知，这时UI线程应刷新相应的控件,因为Realm的自动更新特性，无需重新查询数据。
 **/
